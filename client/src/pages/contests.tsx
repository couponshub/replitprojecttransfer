...
import {
  Trophy, Lock, Users, ChevronLeft, Store, Gift, Tag, Sparkles
} from "lucide-react";
          ...
          <div className={`px-4 py-3 flex items-center gap-2 ${
            ...
            {c.status === "completed" && c.winner_user_name ? (
              <>
                ...
                <span className="text-xs font-black text-amber-900 uppercase tracking-wider">Prize Unlocked!</span>
              </>
            ) : (
              <>
          ...
          <div className="p-4 bg-white dark:bg-gray-900">
            {c.status === "completed" && c.winner_user_name ? (
              <div className="flex flex-col gap-3">
                {/* Winner announcement */}
                <div className="flex items-center gap-3 p-3 rounded-2xl bg-gradient-to-r from-amber-50 to-yellow-50 dark:from-amber-900/20 dark:to-yellow-900/20 border border-amber-200 dark:border-amber-700/50">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center shrink-0">
                    <Trophy className="w-5 h-5 text-white" />
                  </div>
                  <div>
                    <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wider">Winner</p>
                    <p className="font-black text-amber-800 dark:text-amber-300 text-lg leading-tight" data-testid="text-winner-name">{c.winner_user_name}</p>
                    <p className="text-xs text-muted-foreground">Slot #{c.winner_slot_number}</p>
                  </div>
                </div>
                {/* Coupon reveal */}
                {c.attached_coupon && (
                  <div className="flex items-center gap-3 p-3 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-700/50">
                    ...
                    <div>
                      <p className="font-black text-emerald-800 dark:text-emerald-300 text-sm">{c.attached_coupon.code}</p>
                      <p className="text-xs text-emerald-600 dark:text-emerald-400">
                        {c.attached_coupon.discount_type === "percentage"
                          ? `${c.attached_coupon.discount_value}% off`
                          : c.attached_coupon.discount_type === "flat"
                          ? `₹${c.attached_coupon.discount_value} off`
                          : c.attached_coupon.discount_type || "Special Discount"}
                      </p>
                    </div>
                  </div>
                )}
