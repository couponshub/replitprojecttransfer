...
import { ShoppingCart, Trash2, Plus, Minus, Tag, CheckCircle, ArrowLeft, Package } from "lucide-react";

export default function CartPage() {
  const [, navigate] = useLocation();
  const { items, removeItem, updateQuantity, clearCart, total, shopId } = useCart();
  ...
  const validateCoupon = async () => {
    ...
    try {
      ...
      toast({ title: `Coupon applied! Saving ₹${discount.toFixed(0)}` });
    } catch (err: any) {
      toast({ title: err.message, variant: "destructive" });
    } finally {
              ...
              <Card key={item.id} className="rounded-2xl border-0 shadow-md" data-testid={`card-cart-item-${item.id}`}>
                <CardContent className="p-4">
                  <div className="flex items-center gap-4">
                    <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-700 flex items-center justify-center shrink-0">
                  ...
                  <div className="flex items-center justify-between p-3 bg-emerald-50 dark:bg-emerald-950/30 rounded-xl">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-emerald-600 dark:text-emerald-500">Saving ₹{discount.toFixed(0)}</p>
                      </div>
                    </div>
                    <button
