                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-4 mt-2">
                              <div className="flex items-center justify-between p-3 rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30">
                                ...
                                <button
                                  type="button"
                                  onClick={() => setShopForm({ ...shopForm, delivery_fee_enabled: !shopForm.delivery_fee_enabled })}
                                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${shopForm.delivery_fee_enabled ? "bg-emerald-600" : "bg-gray-200 dark:bg-gray-700"}`}
                                >
                                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${shopForm.delivery_fee_enabled ? "translate-x-6" : "translate-x-1"}`} />
                                </button>
                              </div>
                              <div className="flex flex-col gap-1.5">
                                ...
                                <Input
                                  type="number"
                                  placeholder="0.00"
                                  value={shopForm.delivery_fee_amount || ""}
                                  onChange={e => setShopForm({ ...shopForm, delivery_fee_amount: e.target.value })}
                                  disabled={!shopForm.delivery_fee_enabled}
                  ...
                  <div className="flex flex-col gap-5 pb-2">
                    ...
                    {couponForm.type === "combo" && (
                      <div className="flex flex-col gap-3 p-3 rounded-xl border-2 border-orange-200 dark:border-orange-800 bg-orange-50/40 dark:bg-orange-950/20">
                        ...
                        <p className="text-[10px] text-muted-foreground -mt-1">Products add cheyyi + anni items ki oka combo price set cheyyi. Customer claim chestunte anni items original price lo cart ki add avutayi, combo price total avutundi.</p>
                        ...
                        <div className="flex flex-col gap-1.5 max-h-48 overflow-y-auto pr-1">
                          {(products as any[]).filter(p => !couponProdSearch || p.name.toLowerCase().includes(couponProdSearch.toLowerCase())).map(p => {
                            ...
                            return (
                              <div key={p.id} className={`flex items-center gap-2 p-2.5 rounded-xl border-2 transition-all ${inCombo ? "border-orange-400 bg-orange-50 dark:bg-orange-950/30" : "border-gray-200 dark:border-gray-700"}`} data-testid={`combo-item-${p.id}`}>
                                <div className="flex-1 min-w-0">
                                ...
                                {!inCombo ? (
                                  <button type="button" onClick={() => setBundleItems([...bundleItems, { product_id: p.id, name: p.name, custom_price: String(p.price || "0"), quantity: 1 }])}
                                    className="text-[10px] px-2.5 py-1 rounded-lg border border-orange-400 text-orange-600 hover:bg-orange-100 font-semibold shrink-0" data-testid={`combo-add-${p.id}`}>+ Add</button>
                                ) : (
                                  <button type="button" onClick={() => setBundleItems(bundleItems.filter((_, i) => i !== existingIdx))}
                                    className="text-[10px] px-2 py-1 rounded-lg border border-red-200 text-red-500 hover:bg-red-50 font-semibold shrink-0" data-testid={`combo-remove-${p.id}`}>Remove</button>
                                )}
                              </div>
                            );
                          })}
                        ...
                        <div className="flex flex-col gap-1.5">
                          ...
                          <p className="text-[10px] text-muted-foreground -mt-1">Anni items kalisi yentha ki isthunnam ante idi enter cheyyi</p>
                          <div className="relative">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-bold text-muted-foreground">₹</span>
                            <Input type="number" min="0" placeholder="e.g. 299" value={couponForm.value}
                              onChange={e => setCouponForm((f: any) => ({ ...f, value: e.target.value }))}
                              className="rounded-xl pl-8 font-semibold" data-testid="input-combo-total-price" />
                          </div>
                        </div>
                        {/* Combo Summary */}
                        {bundleItems.length > 0 && couponForm.value && (
                          <div className="bg-orange-100 dark:bg-orange-900/30 rounded-xl p-3">
                            {(() => {
                              ...
                              const comboPrice = parseFloat(couponForm.value || "0");
                              const saved = totalMRP - comboPrice;
                              return (
                                <div className="flex justify-between items-center">
                                  <div>
                                    <p className="text-[10px] text-orange-700 dark:text-orange-300 font-semibold">{bundleItems.reduce((s, b) => s + (b.quantity || 1), 0)} item{bundleItems.length !== 1 ? "s" : ""} in combo</p>
